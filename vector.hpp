...
/*#include "containers_test/iterators/random_access_iterator.hpp"
#include "containers_test/iterators/reverse_iterator.hpp"
#include "containers_test/utils/iterator_traits.hpp"*/
...
namespace ft {
	...
	public:
		...
		iterator insert (iterator position, const value_type& val) {
			...
			if (_cap == _sz) {
				try {
					reserve(2 * _cap + (_cap == 0));
				}
				catch (...) {
					throw;
				}
			}
			for (size_type i = static_cast<size_type>(diff); i + 1 < _sz; ++i) {
				try {
					_arr[i + 1] = _arr[i];
				}
				catch (...) {
			...
			try {
				_alloc.construct(_arr + _sz, _arr[_sz - 1]);
			}
			catch (...) {
				throw;
			}
	...
	private:
		T* reserve_for_insert (size_type n, Allocator alloc, T* arr, size_type
		sz) {
		...
		diff, Allocator alloc, const T& val) {
			for (size_t i = sz + n - 1; i > sz - 1; --i) {
				try {
					alloc.construct(arr + i, arr[i - n]);
				}
				catch (...) {
					for (size_type j = sz + n; j > i; --j) {
						alloc.destroy(arr + j);
					}
					throw;
				}
			}
			for (size_type i = sz - 1; i > static_cast<size_type>(diff) &&
									  sz != 0; --i) {
				try {
					arr[i] = arr[i - n];
				}
			...
			for (size_type i = 0; i < n; ++i) {
				try {
					alloc.construct(arr + diff + i, val);
				}
				catch (...) {
					for (size_type j = 0; j < i; ++j) {
						alloc.destroy(arr + diff + j);
					}
					throw;
				}
			}
		...
		void actions_iter_insert(size_type n, size_type sz, T* arr, difference_type diff,
								 Allocator alloc, InputIterator first) {
			for (size_t i = sz + n - 1; i > sz - 1; --i) {
				try {
					alloc.construct(arr + i, arr[i - n]);
				}
			...
			for (size_type i = 0; i < static_cast<size_type>(n); ++i,
					++first) {
				try {
					alloc.construct(arr + diff + i, *first);
				}
				catch (...) {
					for (size_type j = 0; j < i; ++j) {
						alloc.destroy(arr + diff + j);
